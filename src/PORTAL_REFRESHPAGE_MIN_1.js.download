...
function updatePglt(pgltName, responseTxt, fade) {
 ...
 pgltBody.innerHTML = pgltBodyHTML; if(typeof(fixPageletLinksById) != 'undefined') fixPageletLinksById(pgltName); if (minState) {pgltBody.childNodes[0].style.display = 'none';}
...
function pgltAction(name, url, fade, force) {
 ...
 {
 var next_var = win_href.indexOf("&",pos_cnode + cnode_len); var cnode = win_href.substring(pos_cnode + cnode_len + 1, next_var); url += "&pgltnode=" + cnode; }
 }
...
function addPgltParam(name){
 ...
 addParam += "&bNoGlobal=Y";  return addParam;}
...
function loadAllPgltData(cmdReload, t){
 ...
 var bcList = document.querySelector("#pthbcUlScroll"); if (bcList && bcList.children.length <= 0) { resetHistory(); } 

 
 backNavigation.setCookie();  var tabLabel = ptUtil.id("ptdashboardlabel"); if (tabLabel) {
 tabLabel = tabLabel.innerHTML; } else {
 tabLabel = "Home";  if (!isWorkCenter()) { resetHistory(); }
 }
 ...
 typeof ptUtil.id("ptdashboardlabel") != "undefined" && ptUtil.id("ptdashboardlabel")) {
 ...
 if (typeof(bcUpdater) != "undefined" && bcUpdater && !document.getElementById("remotedashboard")) {
...
function updateWindowTitleFromFanPage() {
 ...
 var ptWindowTitle = document.getElementsByTagName("title")[0].textContent; if (typeof(ptWindowTitle) != 'undefined' && ptWindowTitle != null && ptWindowTitle != "")
 {
